...
class Server(object):
    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def room_description(self, room_number):
        ...
        return ["You are in the room with wood floors.",
                "You are in the room with carpet.",
                "you are in the room with tile.",
                "You are in the room with no floor."][room_number]
    ...
    def get_input(self):
        ...
        """
        received = b''
        #continue getting until no mmore \n
        while b'\n' not in received:
            received += self.client_connection.recv(16)
        #converts to unicode and strips off new line character
    ...
    def route(self):
        ...
        # "say" commands.
        if self.input_buffer.startswith("move") == True:
            self.move(self.input_buffer[5:])
        if self.input_buffer.startswith("say") == True:
